...
public final class AiRuntime {
    ...
    public void bootstrap() {
        ...
        Job job = new Job("Verify AI Code Assistant connection") {
            ...
            protected IStatus run(IProgressMonitor monitor) {
                try {
                    testConnection(activeConnection, activeModelPreference, monitor);
    ...
    public synchronized void invalidateDraftConfiguration() {
        ...
        snapshot.set(new RuntimeSnapshot(
            ConnectionStatus.UNVERIFIED,
            ModelStatus.UNRESOLVED,
            "",
            "Configuration changed — test the connection again."
        ));
    }
    ...
    public synchronized void invalidateDraftModel() {
        ...
        ConnectionStatus connection = current.connectionStatus() == ConnectionStatus.OK
            ? ConnectionStatus.OK
            : ConnectionStatus.UNVERIFIED;
        snapshot.set(new RuntimeSnapshot(
            connection,
            ModelStatus.UNRESOLVED,
            "",
            "Model selection changed — resolve the model again."
        ));
    }

    /** Re-resolve Auto mode from the catalog already loaded by Test/Refresh. */
    public synchronized void resolveAutoFromCatalog() {
        ...
        activeModelPreference = new ModelPreference(
            ModelSelectionMode.AUTO,
            activeModelPreference.manualModelId(),
            resolved,
            activeModelPreference.lastKnownGoodModel()
        );
        RuntimeSnapshot current = snapshot.get();
        if (current.connectionStatus() == ConnectionStatus.OK) {
    ...
    public ConnectionTestReport testConnection(
        ConnectionConfig connection,
        ModelPreference modelPreference,
        ...
        try {
            ...
            if (modelPreference.mode() == ModelSelectionMode.AUTO) {
                activeModelPreference = new ModelPreference(
                    ...
                    modelPreference.manualModelId(),
                    resolved,
                    modelPreference.lastKnownGoodModel()
                );
            }
            ...
            if (error.statusCode() == 404) {
                catalog = List.of();
                String manual = modelPreference.mode() == ModelSelectionMode.MANUAL
                    ? modelPreference.manualModelId()
                    : "";
                ...
                snapshot.set(new RuntimeSnapshot(
                    ...
                    modelStatus,
                    manual,
                    "Endpoint does not support listing models."
                ));
                return report(
                    !manual.isBlank(), true, null, false, 0, manual, started,
                    "Endpoint does not support /models; enter a model ID manually."
                );
            }
    ...
    public CompletionResponse complete(CodeContext context, IProgressMonitor monitor, boolean singleLine)
        ...
        String model = state.resolvedModelId();
        ...
        try {
            ...
            String fallback = resolveModel(activeModelPreference, catalog, Set.of(model));
            if (fallback.isBlank()) {
    ...
    private String resolveModel(
        ...
        if (preference.mode() == ModelSelectionMode.MANUAL) {
            return excluded.contains(preference.manualModelId()) ? "" : preference.manualModelId();
        }
    ...
    private void markKnownGood(String model) {
        ...
        activeModelPreference = new ModelPreference(
            activeModelPreference.mode(),
            activeModelPreference.manualModelId(),
            activeModelPreference.lastResolvedAutoId(),
            model
        );
    }

    private void updateRuntimeError(ApiException error) {
        if (error.statusCode() == 401 || error.statusCode() == 403 || error.statusCode() == 0) {
            snapshot.set(new RuntimeSnapshot(ConnectionStatus.ERROR, ModelStatus.UNRESOLVED, "", error.getMessage()));
        } else if (error.isModelResolutionError()) {
            snapshot.set(new RuntimeSnapshot(ConnectionStatus.OK, ModelStatus.ERROR, "", error.getMessage()));
        }
    }

    private void ensureCurrent(long requestGeneration) {
        if (requestGeneration != generation.get()) throw new OperationCanceledException();
    }

    private static ConnectionTestReport report(
        boolean success,
        boolean reachable,
        ...
        return new ConnectionTestReport(
            success, reachable, authentication, catalogSupported, modelCount, model, latency, message
        );
    }
}
