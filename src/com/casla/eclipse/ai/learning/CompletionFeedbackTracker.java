...
public final class CompletionFeedbackTracker {
    ...
    private record Pending(String objectKey, String structureHint, String contextBucket, String model, int offset, String text, int matchedChars, long createdAt) {}
    ...
    public synchronized void generated(CodeContext context, CompletionResponse response) {
        ...
        pending = new Pending(clean(context.filePath()), clean(context.structureHint()), bucket, model, context.cursorOffset(), text, 0, System.currentTimeMillis());
    ...
    public synchronized void documentChanged(String objectKey, String language, DocumentEvent event) {
        ...
        if (event.getLength() != 0 || event.getOffset() != expectedOffset || inserted.isEmpty()) {
            record(current.model(), current.contextBucket(), FeedbackEvent.DISMISSED); pending = null; return;
        }
        ...
        if (newMatched >= current.text().length()) {
            ...
            if (eventType != FeedbackEvent.TYPED_MATCH) {
                acceptedRegion = new AcceptedRegion(key, current.model(), current.contextBucket(), current.offset(), current.text().length(), System.currentTimeMillis() + EDIT_WINDOW_MS, false);
                if (eventType == FeedbackEvent.ACCEPT_FULL) {
                    AdaptiveLearningStore.get().rememberAcceptedExample(key, current.structureHint(), current.text(), current.model());
                }
            }
            pending = null;
            return;
        ...
        if (eventType == FeedbackEvent.ACCEPT_WORD || eventType == FeedbackEvent.ACCEPT_LINE) {
            record(current.model(), current.contextBucket(), eventType);
            acceptedRegion = new AcceptedRegion(key, current.model(), current.contextBucket(), current.offset(), newMatched, System.currentTimeMillis() + EDIT_WINDOW_MS, false);
        }
        pending = new Pending(current.objectKey(), current.structureHint(), current.contextBucket(), current.model(), current.offset(), current.text(), newMatched, current.createdAt());
    }

    public synchronized void dismissPending() {
        expire(); if (pending == null) return;
        record(pending.model(), pending.contextBucket(), FeedbackEvent.DISMISSED); pending = null;
    }
    ...
    private void detectEditAfterAccept(String objectKey, DocumentEvent event) {
        ...
        acceptedRegion = new AcceptedRegion(region.objectKey(), region.model(), region.contextBucket(), region.offset(), region.length(), region.expiresAt(), true);
    }

    private void expire() {
        long now = System.currentTimeMillis();
        if (pending != null && now - pending.createdAt() > PENDING_TTL_MS) { record(pending.model(), pending.contextBucket(), FeedbackEvent.DISMISSED); pending = null; }
        if (acceptedRegion != null && acceptedRegion.expiresAt() < now) acceptedRegion = null;
    }

    private static void record(String model, String bucket, FeedbackEvent event) { AdaptiveLearningStore.get().recordFeedback(model, bucket, event); }
    ...
    static FeedbackEvent classifyInsertion(String remaining, String inserted, int alreadyMatched) {
        ...
        String line = nextLine(remaining); if (inserted.length() > 1 && inserted.equals(line)) return FeedbackEvent.ACCEPT_LINE;
        String word = nextWord(remaining); if (inserted.length() > 1 && inserted.equals(word)) return FeedbackEvent.ACCEPT_WORD;
    ...
    private static String nextWord(String text) {
        ...
        if (i < text.length() && Character.isJavaIdentifierPart(text.charAt(i))) while (i < text.length() && Character.isJavaIdentifierPart(text.charAt(i))) i++;
        else if (i < text.length()) while (i < text.length() && !Character.isWhitespace(text.charAt(i)) && !Character.isJavaIdentifierPart(text.charAt(i))) i++;
